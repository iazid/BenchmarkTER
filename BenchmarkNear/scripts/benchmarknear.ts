...
// Recherche et validation du dossier de credentials
...
const VIEW_RUNS = 5;
const TX_GAS = 30_000_000_000_000n; // 30 TGas
const TX_DEPOSIT = 0n;
...
async function initAccount(): Promise<Account> {
  ...
  console.log(`Clé JSON attendue : ${path.join(CREDENTIALS_DIR, NETWORK_ID, `${CONTRACT_ACCOUNT}.json`)}`);
  ...
  console.log(`Clé trouvée pour ${CONTRACT_ACCOUNT}: ${keyPair.getPublicKey().toString()}`);
...
async function measureView(name: string, contract: any, method: string, args: any): Promise<Result> {
  ...
  const variance = latencies.reduce((s, v) => s + Math.pow(v - mean, 2), 0) / (n - 1);
  ...
  console.log(
    `[VIEW  ${name}] mean=${result.latencyMs}ms ±${result.ci95ms}ms (std=${result.stdLatencyMs}ms) over ${n} runs`
  );
  return result;
}
...
async function main() {
  // 1. Initialisation
  let account: Account;
  try { account = await initAccount(); } catch (e: any) { console.error(e.message); process.exit(1); }

  // 2. Déploiement
  const results: Result[] = [];
  try { results.push(await deployContract(account)); } catch (e: any) { console.error('Deploy error:', e.message); process.exit(1); }

  // 3. Instanciation du contrat
  ...
  for (const [name,args] of viewTests) results.push(await measureView(name, contract, name, args));

  // 5. Affichage console & export CSV
  console.log('\n=== Résultats complets ===');
  console.table(results, ['testName','actualGasUsed','latencyMs','stdLatencyMs','ci95ms','result','extra']);

  const ts = new Date().toISOString().replace(/:/g,'-');
  const outPath = path.resolve(__dirname, `../results/near-benchmark_${ts}.csv`);
